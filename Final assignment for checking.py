...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        # use groupby to create relevant data for plotting
        average_sales = recession_data('Vehicle_Type')('Automobile_Sales').mean().reset_index()                           
        R_chart2  = dcc.Graph(
            figure=px.bar(average_sales,
            x='Vehicle_Type',
            y='Automobile_Sales',
            title='Average Automobile Sales for different types of autombiles'))
        
# Plot 3 Pie chart for total expenditure share by vehicle type during recessions
        # use groupby to create relevant data for plotting
        exp_rec= recession_data.groupby('Vehicle_Type')['Advertising_Expenditure'].sum()
        R_chart3 = dcc.Graph(
            figure=px.pie(exp_rec.values,
            labels=exp_rec.index,
            autopct='%1.1f%%', 
            startangle=90,
            title='Total Advertising Expenditure during Recessions based on Vehicle Type'))
...
# Plot 2 Total Monthly Automobile sales using line chart.
        ...
        x='Month',
        y='Automobile_Sales',
        title='Monthly Automobile_Sales'))

            # Plot bar chart for average number of vehicles sold during the given year
        avr_vdata=yearly_data.groupby('year')['Automobile Sales'].mean().reset_index()
        ...
        Y_chart4 = dcc.Graph(figure=px.pie(exp_data.values,
                                label=exp_data.index,
                                autopct='%1.1f%%', 
                                startangle=90,
                                title='Total Advertising Expenditure during Recessions based on Vehicle Type'))
